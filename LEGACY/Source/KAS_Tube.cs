...
public class KAS_Tube : MonoBehaviour {
  ...
  public void Load() {
    ...
    if (srcJointType != tubeJointType.None) {
      ...
      srcSphere.transform.localRotation =
          Quaternion.identity * Quaternion.Euler(new Vector3(90f, 0f, 0f));
      ...
      if (srcJointType == tubeJointType.Joined) {
        ...
        ScaleBetweenPoints(srcTubeSphere.transform, srcNode.position, srcSphere.transform.position,
                           tubeAngle, srcTubeSphereMR.material, tubeTexTilingOffset);
      }
    }
    ...
    if (tgtJointType != tubeJointType.None) {
      ...
      tgtSphere.transform.localRotation =
          Quaternion.identity * Quaternion.Euler(new Vector3(90f, 0f, 0f));
      ...
      if (tgtJointType == tubeJointType.Joined) {
        ...
        ScaleBetweenPoints(tgtTubeSphere.transform, tgtNode.position, tgtSphere.transform.position,
                           tubeAngle, tgtTubeSphereMR.material, tubeTexTilingOffset);
      }
    }
    UpdateTube();
    tubeLoaded = true;
  ...
  public void UnLoad() {
    ...
    if (tgtTubeSphere) {
      Destroy(tgtTubeSphere);
    }
    if (srcTubeSphere) {
      Destroy(srcTubeSphere);
    }
    tubeLoaded = false;
  }
  ...
  void UpdateTube() {
    ...
    ScaleBetweenPoints(tube.transform, tmpSrcNode, tmpTgtNode,
                       tubeAngle, tubeMR.material, tubeTexTilingOffset);
  }

  public void SetColor(Color colorToSet) {
    if (tubeMR) {
      tubeMR.material.color = colorToSet;
    }
    if (srcSphereMR) {
      srcSphereMR.material.color = colorToSet;
    }
    if (srcTubeSphereMR) {
      srcTubeSphereMR.material.color = colorToSet;
    }
    if (tgtSphereMR) {
      tgtSphereMR.material.color = colorToSet;
    }
    if (tgtTubeSphereMR) {
      tgtTubeSphereMR.material.color = colorToSet;
    }
  }
  ...
  public void SetTexture(Texture2D textureToSet) {
    if (tubeMR) {
      tubeMR.material.mainTexture = textureToSet;
    }
    if (srcSphereMR) {
      srcSphereMR.material.mainTexture = textureToSet;
    }
    if (srcTubeSphereMR) {
      srcTubeSphereMR.material.mainTexture = textureToSet;
    }
    if (tgtSphereMR) {
      tgtSphereMR.material.mainTexture = textureToSet;
    }
    if (tgtTubeSphereMR) {
      tgtTubeSphereMR.material.mainTexture = textureToSet;
    }
  }
  ...
  void ScaleBetweenPoints(Transform obj, Vector3 srcPos, Vector3 tgtPos, Vector3 angle,
                          Material material = null, float textureTilingOffset = 0) {
    obj.position = (srcPos + tgtPos) / 2;
    obj.LookAt(tgtPos);
    obj.Rotate(angle, Space.Self);
    obj.localScale = new Vector3(
        obj.localScale.x, Vector3.Distance(obj.position, tgtPos), obj.localScale.z);
    if (material && textureTilingOffset != 0) {
      material.mainTextureScale = new Vector2(
          material.mainTextureScale.x,
          Vector3.Distance(obj.position, tgtPos) * textureTilingOffset);
    }
  }
