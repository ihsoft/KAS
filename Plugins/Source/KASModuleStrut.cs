...
public class KASModuleStrut : KASModuleAttachCore {
  ...
  public override string GetInfo() {
    ...
    sb.Append("<b>Compatibility</b>: ");
    sb.AppendLine(type);
    sb.AppendFormat("<b>Maximum length</b>: {0:F0}m", maxLenght);
    sb.AppendLine();
    sb.AppendFormat("<b>Maximum angle</b>: {0:F0}", maxAngle);
    sb.AppendLine();
    sb.AppendFormat("<b>Strength</b>: {0:F0}", breakForce);
    sb.AppendLine();
    if (allowDock) {
      sb.AppendLine();
  ...
  private IEnumerator<YieldInstruction> WaitAndRedock() {
    ...
    if (part && vessel && linked && linkedStrutModule.vessel != this.vessel && allowDock) {
      KAS_Shared.DebugWarning("WaitAndRedock(strut) Source and target vessel are different,"
                              + " docking strut... (allowDock = true)");
  ...
  private void SetEvaLink() {
    ...
    if (!FlightGlobals.ActiveVessel.isEVA) {
      ScreenMessages.PostScreenMessage("Active vessel is not eva !",
  ...
  private bool CheckLink(Transform srcTransform, Transform tgtTransform, bool checkTgtAngle) {
    ...
    // Check max angle
    if (KAS_Shared.GetAngleFromDirAndPoints(
        srcTransform.forward, srcTransform.position, tgtTransform.position) > maxAngle) {
      maxAngleReached = true;
    }
    if (checkTgtAngle
        && KAS_Shared.GetAngleFromDirAndPoints(
            tgtTransform.forward, tgtTransform.position, srcTransform.position) > maxAngle) {
      maxAngleReached = true;
    }
  ...
  [KSPEvent(name = "ContextMenuTogglePump", active = false, guiActiveUnfocused = true,
            guiActive = true, unfocusedRange = 2f, guiName = "Pump Here")]
  ...
  [KSPEvent(name = "ContextMenuLink", active = true, guiActiveUnfocused = true, guiActive = false,
            unfocusedRange = 2f, guiName = "Link")]
  ...
  [KSPEvent(name = "ContextMenuUnlink", active = true, guiActiveUnfocused = false,
            guiActive = false, unfocusedRange = 2f, guiName = "Unlink")]
