...
static public class KAS_Shared {
  ...
  public static void MoveAbove(Transform fromTransform, Vector3 fromLocalPos,
                               Vector3 fromOrientation, RaycastHit hit) {
    fromTransform.rotation = Quaternion.FromToRotation(fromOrientation, -hit.normal);
    fromTransform.position =
        fromTransform.position - (fromTransform.TransformPoint(fromLocalPos) - hit.point);
  }

  public static void MoveAlignLight(Vessel fromVessel, Transform fromTransform,
                                    Vessel toVessel, Transform toTransform) {
    toVessel.SetRotation(toVessel.transform.rotation);
    fromVessel.SetRotation(Quaternion.FromToRotation(
        fromTransform.forward, -toTransform.forward) * fromVessel.transform.rotation);
    fromVessel.SetPosition(
        fromVessel.transform.position - (fromTransform.position - toTransform.position), true);
  }
  ...
  public static void MovePartWith(Part rootPart, List<Part> moveWithParts, Vector3 position,
                                  Quaternion rotation) {
  ...
  public static void MoveRelatedTo(Transform fromTransform, Transform toTransform,
  ...
  public static List<KASModuleWinch> GetAllWinch(Vessel fromVessel = null) {
    ...
    var winches = new List<KASModuleWinch>(
        GameObject.FindObjectsOfType(typeof(KASModuleWinch)) as KASModuleWinch[]);
  ...
  public static bool MouseIsOverWindow(Rect guiWindowRect) {
    return guiWindowRect.Contains(Mouse.screenPos);
  }
  ...
  public static void RemoveAttachJointBetween(Part part1, Part part2) {
    if (part1.attachJoint
        && ((part1.attachJoint.Host == part1 && part1.attachJoint.Target == part2)
            || (part1.attachJoint.Host == part2 && part1.attachJoint.Target == part1))) {
      part1.attachJoint.DestroyJoint();
    }
    if (part2.attachJoint
        && ((part2.attachJoint.Host == part2 && part2.attachJoint.Target == part1)
            || (part2.attachJoint.Host == part1 && part2.attachJoint.Target == part2))) {
  ...
  public static void UpdateChildsOrgPos(Part p, bool includeItself = false) {
    if (includeItself) {
      p.UpdateOrgPosAndRot(p.localRoot);
    }
    List<Part> pcs = KAS.KAS_Shared.GetAllChilds(p);
    foreach (Part pc in pcs) {
  ...
  public static void SetPartsPosition(Part rootPart, List<Part> parts, Vector3 position,
                                      bool usePristineCoords) {
    if (!usePristineCoords) {
      Vector3 vector = position - rootPart.transform.position;
      foreach (Part p in parts) {
        if (p == rootPart || p.physicalSignificance != Part.PhysicalSignificance.FULL) {
          continue;
        }
  ...
  public static KASModuleWinch GetWinchModuleGrabbed(Vessel evaVessel) {
    var allWinches = new List<KASModuleWinch>(
        GameObject.FindObjectsOfType(typeof(KASModuleWinch)) as KASModuleWinch[]);
    foreach (KASModuleWinch winch in allWinches) {
      if (!winch.evaHolderPart) {
        continue;
      }
  ...
  public static void SetPartLocalPosRotFrom(Transform trf, Transform from,
