...
namespace KAS
{
    public class KASModuleTelescopicArm : PartModule
    {
        ...
        [KSPField(guiActive = true, guiName = "Projection distance", guiFormat = "F6", guiUnits = "m")] public float prjDist = 0;
        [KSPField(guiActive = true, guiName = "Projection angle", guiFormat = "F6", guiUnits = "m")] public float prjAngle = 0;
        ...
        private void LoadSections()
        {
            ...
            foreach (ConfigNode sectionNode in node.GetNodes("SECTION"))
            {
                if (sectionNode.HasValue("transformName") && sectionNode.HasValue("lenght"))
                {
                    ...
                    {
                        KAS_Shared.DebugError("LoadSections(TelescopicArm) Section transform " + sectionTransformName + " not found in the model !");
                        continue;
                    }
                    if (!float.TryParse(sectionNode.GetValue("lenght"), out section.lenght))
                    {
                        KAS_Shared.DebugError("LoadSections(TelescopicArm) Unable to parse lenght of the Section : " + sectionTransformName);
                        continue;
                    }
                    ...
                    if (savedSectionsLocalPos.Count > 0) section.savedLocalPos = savedSectionsLocalPos[i];
        ...
        {
            if (!boomHeadLoaded) LoadBoomHead();
        }
        ...
        {
            float dist = Vector3.Distance(this.part.transform.TransformPoint(sections[0].orgLocalPos), sections[0].transform.position);
            ...
            if (extend.active && !extend.full)
            {
                if (KAS_Shared.RequestPower(this.part, powerDrain))
                {
                    ...
                    if (headPos < sectionTotalLenght)
                    {
                        ...
                        if (targetPos < sectionTotalLenght) SetTargetPos(targetPos + speed * TimeWarp.deltaTime);
                        else SetTargetPos(sectionTotalLenght);
                    }
                    else
                    {
                        extend.full = true;
        ...
        private void UpdateRetract()
        {
            if (retract.active && !retract.full)
            {
                if (KAS_Shared.RequestPower(this.part, powerDrain))
                {
                    ...
                    {
                        if (!fxSndMotor.audio.isPlaying) fxSndMotor.audio.Play();
                        if (targetPos > 0) SetTargetPos(targetPos - speed * TimeWarp.deltaTime);
                        else SetTargetPos(0);
                    }
                    else
                    {
        ...
        {
            if (headPos == Round(targetPos)) return;
            ...
            foreach (KeyValuePair<int, SectionInfo> section in sections)
            {
                if (section.Key + 1 >= sections.Count) return;

                sumLenght += section.Value.lenght;
                if (headPos > sumLenght && sectionIndex == activeSection)
                {
                    KAS_Shared.DebugLog("UpdateSections(TelescopicArm) - Extend | Change section to : " + sectionIndex);
                    sections[section.Key + 1].transform.parent = section.Value.transform;
                    sectionIndex += 1;
                    disableSectionCollision();
                ...
                {
                    KAS_Shared.DebugLog("UpdateSections(SlideMotor) - Retract | Change section to : " + sectionIndex);
        ...
        private Vector3 GetOrientedLocalPos(float pos)
        {
            ...
            if (direction.x != 0)
            {
                if (driveWay > 0) orientedLocalPos = new Vector3(pos, 0, 0);
                if (driveWay < 0) orientedLocalPos = new Vector3(-pos, 0, 0);
            }
            else if (direction.y != 0)
            {
                if (driveWay > 0) orientedLocalPos = new Vector3(0, pos, 0);
                if (driveWay < 0) orientedLocalPos = new Vector3(0, -pos, 0);
            }
            else if (direction.z != 0)
            {
                if (driveWay > 0) orientedLocalPos = new Vector3(0, 0, pos);
                if (driveWay < 0) orientedLocalPos = new Vector3(0, 0, -pos);
            }
