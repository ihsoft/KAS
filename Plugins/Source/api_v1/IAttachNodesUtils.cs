...
public interface IAttachNodesUtils {
  ...
  /// <remarks>This method ensures that the found node is properly initialized. As of KSP 1.1.3.1289
  /// owner of the node can be <c>null</c>. The node returned by this methid is guaranteed to have
  /// all the members set as expected.</remarks>
  ...
  /// <remarks>Attach node transform is located at the attach node position and looks in the same
  /// direction. The returned transform is a child of the owning part transform, so it could be used
  /// as an anchor in relational 3D calculations.</remarks>
