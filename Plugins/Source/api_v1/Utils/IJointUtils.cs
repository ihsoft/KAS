...
public interface IJointUtils {
  ...
  /// <remarks>It's not the same as creating a default joint. The state is consistent but different
  /// from the default:
  /// <list type="">
  ...
  /// <item>Connected body is <i>not</i> touched. Connection, if any, won't be broken on the reset.
  /// </item>
  /// </list>
  /// <para>Use this method before setting up a new or existing joint. By resetting the joint you
  /// ensure it's in a consistent state, and further adjustments will always give the same result
  /// regardless to how the joint was created and what components were affecting it.</para>
  /// </remarks>
  ...
  /// <remarks>It's a <see href="http://docs.nvidia.com/gameworks/content/gameworkslibrary/physx/guide/Manual/Joints.html#prismatic-joint">standard PhysX configuration</see>.
  /// Main axis is set to Z. Moving along it is allowed but can be constrained by a spring and
  /// limit. Drive mode is set to <see cref="JointDriveMode.Position"/>.
  /// <para>Only main axis linear settings are changed. Consider using <see cref="ResetJoint"/> to
  /// eliminate sideffects from the previous settings of the joint.</para>
  /// <para>Original positions of the bodies are calculated on the first <c>FixedUpdate()</c>
  /// following the joint setup. These positions will be used to calculate relative movements of the
  /// bodies.</para>
  /// <para>Pure prismatic joint assumes 5 out of the 6 degrees of freedom to be locked (everything,
  ...
  /// <paramref name="distanceLimit"/> is <c>0</c> the main axis linear movement mode is set to
  /// <see cref="ConfigurableJointMotion.Locked"/>. If you plan to change force/limit don't forget
  /// to update the modes as well.</item>
  ...
  /// need to re-apply them when changing mode.</para>
  /// </remarks>
  ...
  /// <remarks>It's a <see href="http://docs.nvidia.com/gameworks/content/gameworkslibrary/physx/guide/Manual/Joints.html#spherical-joint">standard PhysiX configuration</see>.
  /// Main axis is set to Z, and angular rotation around it is completely unrestricted. Secondary
  /// axes are X&amp;Y can be restricted by applying spring force and/or limits. Drive mode is set
  /// to <see cref="JointDriveMode.Position"/>.
  /// <para>Only angular settings are set. If joint had linear constraints defined they will stay
  /// unchanged. Consider using <see cref="ResetJoint"/> to eliminate sideffects from the previous
  /// settings of the joint.</para>
  /// <para>Original rotations of the bodies are calculated on the first <c>FixedUpdate()</c>
  /// following the joint setup. These rotations will be used to calculate relative movements of the
  /// bodies.</para>
  /// <para>Pure spherical joint assumes 3 out of the 6 degrees of freedom to be locked (all the
  /// three axes linear motions). Consider setting
  /// <see cref="ConfigurableJoint.enablePreprocessing"/> to <c>true</c> since it may improve PhysX
  /// performance.</para>
  /// <para>For performance reasons some parameters combindations may result in different angular
  /// modes:
  /// <list>
  /// <item>When <paramref name="springForce"/> is <c>Infinite</c> or <paramref name="angleLimit"/>
  /// is <c>0</c> Y&amp;Z rotation modes are set to <see cref="ConfigurableJointMotion.Locked"/>. If
  /// you plan to change force/limit don't forget to update the modes as well.</item>
  /// <item>When <paramref name="springForce"/> is <c>0</c> and <paramref name="angleLimit"/> is
  ...
  /// </remarks>
  /// <param name="joint">Joint to setup.</param>
  /// <param name="springForce">Torque force to apply when joint angle deviates from the
  /// original.</param>
  /// <param name="springDamperRatio">Percentage of the torque force to use for dampering
  /// oscillation effect.</param>
  /// <param name="maxSpringForce">Maximum torque force to apply when joint angle deviates from the
  /// original.</param>
  /// <param name="angleLimit">Maximum rotation angle (degrees) around Y or Z axis.</param>
  /// <param name="angleLimitForce">Force to apply to keep joint in limits. If it's <c>0</c> then
  /// the limit is impassible.</param>
  /// <param name="angleLimitDamperRatio">Percentage of the limit force to use for dampering
  /// oscillation effect.</param>
