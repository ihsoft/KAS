...
public class KASModuleMagnet : KASModuleAttachCore {
  ...
  public override string GetInfo() {
    ...
    sb.AppendFormat("<b>Magnet strength</b>: {0:F0}", breakForce);
    sb.AppendLine();
  ...
  void UpdateMagnet() {
    ...
    if (!KAS_Shared.RequestPower(this.part, powerDrain)) {
      ...
      if (this.part.vessel == FlightGlobals.ActiveVessel) {
        ScreenMessages.PostScreenMessage("Magnet stopped ! Insufficient Power",
  ...
  private void AttachOnCollision(Collision collision, string type) {
    foreach (ContactPoint contact in collision.contacts) {
      ...
      // Info
      KAS_Shared.DebugLog("forward dot is : " + fwdDot + " and lockMinFwdDot is set to : "
                          + minFwdDot);
      KAS_Shared.DebugLog("roll dot is : " + rollDot + " and lockMinRollDot is set to : "
                          + minRollDot);
      KAS_Shared.DebugLog("AttachOnCollision - Collision with unattached part detected : "
                          + p.name + " | at : " + contact.point);
  ...
  [KSPEvent(name = "ContextMenuMagnet", active = true, guiActive = true, guiActiveUnfocused = true,
